 ...
 */
 ...
 static const char* g_toolDescription = 
    "Application usage statistics query tool，提供应用使用时长、事件记录、高频使用时段等查询功能。"
    "用于开发者或系统管理员分析应用使用情况。"
 ...
 int CmdCheckBundleIdle(int argc, char** argv)
 {
     if (CheckHelpFlag(argc, argv)) {
         CLI_LOG("%s check-bundle-idle - 检查指定应用是否处于空闲状态。"
            "用于判断应用是否可被系统优化。仅适用于已安装应用。", g_programName);
         CLI_LOG("");
         CLI_LOG("Usage:");
         CLI_LOG("  %s check-bundle-idle --bundle <bundleName> [--user <userId>]", g_programName);
         CLI_LOG("");
         CLI_LOG("Parameters:");
         ...
         CLI_LOG("");
         CLI_LOG("Examples:");
         CLI_LOG("  %s check-bundle-idle --bundle com.example.app", g_programName);
         CLI_LOG("  %s check-bundle-idle --bundle com.example.app --user 100", g_programName);
         return 0;
     }
 ...
 int CmdCheckBundlePeriod(int argc, char** argv)
 {
     if (CheckHelpFlag(argc, argv)) {
         CLI_LOG("%s check-bundle-period - 检查应用是否在使用时段（仅Native Token可调用）。"
            "用于判断应用活跃状态。不适用于系统应用调用。", g_programName);
         CLI_LOG("");
         CLI_LOG("Usage:");
         CLI_LOG("  %s check-bundle-period --bundle <bundleName> [--user <userId>]", g_programName);
         CLI_LOG("");
         CLI_LOG("Parameters:");
         CLI_LOG("  --bundle <string>      应用包名（必需，格式：com.example.app）");
         CLI_LOG("  --user <integer>       用户ID（可选，默认：-1 表示当前用户）");
         ...
         CLI_LOG("");
         CLI_LOG("Examples:");
         CLI_LOG("  %s check-bundle-period --bundle com.example.app", g_programName);
         CLI_LOG("  %s check-bundle-period --bundle com.example.app --user 100", g_programName);
         CLI_LOG("");
 ...
 int CmdQueryStatsInterval(int argc, char** argv)
 {
     if (CheckHelpFlag(argc, argv)) {
         CLI_LOG("%s query-stats-interval - 按时间间隔查询应用使用统计。"
            ...
            "仅支持有效的时间范围查询。", g_programName);
         CLI_LOG("");
         CLI_LOG("Usage:");
         CLI_LOG("  %s query-stats-interval --interval <type> --begin <timestamp> "
            "--end <timestamp> [--user <userId>]", g_programName);
CLI_LOG("");
    CLI_LOG("Parameters:");
    ...
    CLI_LOG("  --help                 显示帮助信息");
         CLI_LOG("");
         CLI_LOG("Examples:");
         CLI_LOG("  %s query-stats-interval --interval 0 --begin 1609459200000 --end 1609545600000", g_programName);
         CLI_LOG("  %s query-stats-interval --interval 1 --begin 1609459200000 "
            "--end 1610064000000 --user 100", g_programName);
         return 0;
     }
 ...
 int CmdQueryEvents(int argc, char** argv)
 {
     if (CheckHelpFlag(argc, argv)) {
         ...
         CLI_LOG("  %s query-events --begin <timestamp> --end <timestamp> "
            "[--user <userId>] [--max <count>]", g_programName);
CLI_LOG("");
    CLI_LOG("Parameters:");
    CLI_LOG("  --begin <number>       开始时间戳（必需，单位：毫秒）");
    CLI_LOG("  --end <number>         结束时间戳（必需，单位：毫秒）");
    ...
    CLI_LOG("  --help                 显示帮助信息");
         CLI_LOG("");
         CLI_LOG("Examples:");
         CLI_LOG("  %s query-events --begin 1609459200000 --end 1609545600000", g_programName);
         CLI_LOG("  %s query-events --begin 1609459200000 --end 1609545600000 --max 500 --user 100", g_programName);
         return 0;
     }
 ...
 int CmdQueryAppGroup(int argc, char** argv)
 {
     if (CheckHelpFlag(argc, argv)) {
         CLI_LOG("%s query-app-group - 查询应用优先级分组。"
            "用于获取应用在资源调度中的优先级等级。仅适用于已安装应用。", g_programName);
         CLI_LOG("");
         CLI_LOG("Usage:");
         CLI_LOG("  %s query-app-group --bundle <bundleName> [--user <userId>]", g_programName);
         CLI_LOG("");
         CLI_LOG("Parameters:");
         CLI_LOG("  --bundle <string>      应用包名（必需，格式：com.example.app）");
         CLI_LOG("  --user <integer>       用户ID（可选，默认：-1 表示当前用户）");
         ...
         CLI_LOG("");
         CLI_LOG("Examples:");
         CLI_LOG("  %s query-app-group --bundle com.example.app", g_programName);
         CLI_LOG("  %s query-app-group --bundle com.example.app --user 100", g_programName);
         return 0;
     }
 ...
 int CmdQueryHighFreqBundle(int argc, char** argv)
 {
     if (CheckHelpFlag(argc, argv)) {
         CLI_LOG("%s query-high-freq-bundle - 查询高频使用应用列表。"
            ...
            "仅支持有效的查询参数。", g_programName);
         CLI_LOG("");
         CLI_LOG("Usage:");
         CLI_LOG("  %s query-high-freq-bundle [--user <userId>] [--max <count>] [--days <range>]", g_programName);
         CLI_LOG("");
         CLI_LOG("Parameters:");
         CLI_LOG("  --user <integer>       用户ID（可选，默认：-1 表示当前用户）");
         ...
         CLI_LOG("");
         CLI_LOG("Examples:");
         CLI_LOG("  %s query-high-freq-bundle", g_programName);
         CLI_LOG("  %s query-high-freq-bundle --max 50 --days 14", g_programName);
         CLI_LOG("  %s query-high-freq-bundle --user 100 --max 50 --days 14", g_programName);
         return 0;
     }
...
return OutputError("ERR_QUERY_FAILED", "查询高频使用应用失败",
 ...
 int CmdQueryModuleRecords(int argc, char** argv)
 {
     if (CheckHelpFlag(argc, argv)) {
         ...
         CLI_LOG("");
         CLI_LOG("Usage:");
         CLI_LOG("  %s query-module-records [--max <count>] [--user <userId>]", g_programName);
         CLI_LOG("");
         CLI_LOG("Parameters:");
         CLI_LOG("  --max <integer>        最大返回数量（可选，范围：1-1000，默认：1000）");
         CLI_LOG("  --user <integer>       用户ID（可选，默认：-1 表示当前用户）");
         ...
         CLI_LOG("");
         CLI_LOG("Examples:");
         CLI_LOG("  %s query-module-records", g_programName);
         CLI_LOG("  %s query-module-records --max 500", g_programName);
         CLI_LOG("  %s query-module-records --max 500 --user 100", g_programName);
         return 0;
     }
...
CLI_LOG("");
    ...
    CLI_LOG("  --help                 显示帮助信息");
         CLI_LOG("");
         CLI_LOG("Examples:");
         CLI_LOG("  %s query-notification-stats --begin 1609459200000 --end 1609545600000", g_programName);
         CLI_LOG("  %s query-notification-stats --begin 1609459200000 --end 1609545600000 --user 100", g_programName);
         return 0;
     }
 ...
 int CmdQueryHighFreqPeriod(int argc, char** argv)
 {
     if (CheckHelpFlag(argc, argv)) {
         CLI_LOG("%s query-high-freq-period - 查询高频使用时段。"
            "用于获取应用高频使用的小时分布情况。仅支持有效的用户查询。", g_programName);
         CLI_LOG("");
         CLI_LOG("Usage:");
         CLI_LOG("  %s query-high-freq-period [--user <userId>]", g_programName);
         CLI_LOG("");
         CLI_LOG("Parameters:");
         CLI_LOG("  --user <integer>       用户ID（可选，默认：-1 表示当前用户）");
         CLI_LOG("  --help                 显示帮助信息");
         CLI_LOG("");
         CLI_LOG("Examples:");
         CLI_LOG("  %s query-high-freq-period", g_programName);
         CLI_LOG("  %s query-high-freq-period --user 100", g_programName);
         return 0;
     }
 ...
 int CmdQueryLatestUsedTime(int argc, char** argv)
 {
     if (CheckHelpFlag(argc, argv)) {
         CLI_LOG("%s query-latest-used-time - 查询应用今日最后使用时间。"
            "用于获取应用当天最后一次被使用的时间戳。"
            "仅适用于已安装应用。", g_programName);
         CLI_LOG("");
         CLI_LOG("Usage:");
         CLI_LOG("  %s query-latest-used-time --bundle <bundleName> [--user <userId>]", g_programName);
         CLI_LOG("");
         CLI_LOG("Parameters:");
         CLI_LOG("  --bundle <string>      应用包名（必需，格式：com.example.app）");
         CLI_LOG("  --user <integer>       用户ID（可选，默认：-1 表示当前用户）");
         ...
         CLI_LOG("");
         CLI_LOG("Examples:");
         CLI_LOG("  %s query-latest-used-time --bundle com.example.app", g_programName);
         CLI_LOG("  %s query-latest-used-time --bundle com.example.app --user 100", g_programName);
         return 0;
     }
...
return OutputError("ERR_QUERY_FAILED", "查询应用今日最后使用时间失败",
 ...
 int CmdHelp(int argc, char** argv)
 {
     ...
     if (targetCmd.empty()) {
         CLI_LOG("%s - %s", g_programName, g_toolDescription);
         CLI_LOG("");
         CLI_LOG("Usage:");
         ...
         CLI_LOG("  %s check-bundle-idle --help", g_programName);
         CLI_LOG("  %s check-bundle-idle --bundle com.example.app", g_programName);
         CLI_LOG("");
     ...
     char* helpArgv[1] = {nullptr};
     return it->second.handler(1, helpArgv);
 }
 ...
 void InitCommands()
 {
     ...
     g_commands["check-bundle-period"] = {
         "check-bundle-period",
         "检查应用是否在使用时段（仅Native Token）",
         nullptr,
         nullptr,
         nullptr,
         CmdCheckBundlePeriod
     };
 ...
 int main(int argc, char** argv)
 {
     ...
     for (int i = 2; i < argc; i++) {
         if (strcmp(argv[i], "--help") == 0) {
             ...
             if (it != g_commands.end()) {
                 char* helpArgv[2] = {nullptr, const_cast<char*>(cmdName.c_str())};
                 return CmdHelp(2, helpArgv);
             }
         }
     }
     ...
     if (it == g_commands.end()) {
         return OutputError("ERR_INVALID_COMMAND", 
             ...
             std::string("Please run ") + g_programName + " --help to see available commands");
     }
